...
function DueBadge({ label, hide = ['No date'], subtle = false }: DueBadgeProps) {
  if (hide.includes(label)) return null

  const subtleClass =
    subtle && label === 'Due today'
      ? 'border-slate-200/60 bg-slate-50 text-[9px] font-normal tracking-wide text-slate-500'
      : ''

  return (
    <span
      className={`inline-flex shrink-0 items-center rounded-md border px-1.5 py-0.5 text-[10px] font-medium leading-none ${BADGE_STYLES[label]} ${subtleClass}`}
    >
      {label}
    </span>
  )
